...
function Overview({ data, progress, milestone }: { data:WishlistDashboardData; progress:number; milestone:number }) {
  ...
  return <>
    ...
    <div className="stat-grid">
      ...
      <article className="stat-card"><p>Latest reported net</p><strong>{signedCount(latest?.net ?? 0)}</strong><div className="metric-row"><span><i className="add" />{formatCount(latest?.adds ?? 0)} adds</span><span><i className="delete" />{formatCount(latest?.deletes ?? 0)} deletes</span></div></article>
